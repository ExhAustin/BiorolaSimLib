...
% 	Note: GUI is flawed, can only save to video. Use plot_gui.m for better functionality.
...
f = figure('Visible','on','Position',[30,100,1000,575],'Name','Movie_GUI');
...
function Run
	while 1
		...
		if stop_button==0
			t = t_last + direction*(1/fps)*speed_slider;
...
function speed_slider_Callback(source,callbackdata)
	speed_text.String=['Playing speed ',num2str(speed_slider)];
end
...
function SaveVideo_button_Callback(source,callbackdata)
	...
	if isequal(filename,0) || isequal(pathname,0)
		...
		clear FIG
		ww=[1 1 1366 768];
		FIG=figure('Visible','off','Position', ww,'Resize','off','ToolBar','none','WindowStyle','modal','Name','Movie_Save');
